...
`pragma diagnostic ignore=("default", "empty-member")
    ...
    for (auto& diag : compilation.getAllDiagnostics()) {
        if (diag.code == diag::UndeclaredIdentifier) {
            auto diagWithNote = diag;
            diagWithNote.addNote(diag::NoteDeclarationHere, diag.location);
            engine.issue(diagWithNote);
        }
        else {
            engine.issue(diag);
        }
...
module m;
    ...
    engine.addClient(client);
    for (auto& diagnostic : compilation.getAllDiagnostics()) {
        if (diagnostic.code == diag::UndeclaredIdentifier) {
            auto diagnosticWithNote = diagnostic;
            diagnosticWithNote.addNote(diag::NoteDeclarationHere, diagnostic.location);
            engine.issue(diagnosticWithNote);
        }
        else {
            engine.issue(diagnostic);
        }
