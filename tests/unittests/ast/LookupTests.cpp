...
TEST_CASE("Circular symbol re-export with no grounding declaration terminates") {
    // Same as the plain-import cycle above, but the undeclared name is also re-exported
    // in the cycle. This drives the recursion through resolveExports() (not just
    // ExplicitImportSymbol's initialized flag), so it exercises the resolved-flag guard
    // that breaks re-entrant export resolution.
    auto tree = SyntaxTree::fromText(R"(
package a;
    import b::x;
    export b::x;
...
endpackage
    ...
    REQUIRE(diags.size() == 4);
    for (auto& diag : diags)
        CHECK(diag.code == diag::UnknownPackageMember);
}
