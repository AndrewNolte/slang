...
public:
    ...
    /// Print the leading comments for the provided @a node.
    ...
    /// Print the provided @a node to the internal buffer, with the leading comments on the primary
    /// node.
    /// See @a printLeadingComments for detail on what is considered a leading comment.
    /// @return a reference to this object, to allow chaining additional method calls.
    ...
    /// @return a reference to this object, to allow chaining additional method calls.
    SyntaxPrinter& printExcludingLeadingTrivia(const SyntaxNode& node);
